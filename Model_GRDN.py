...
def RDBlock(filters, growth_rate, dense_count):
	def func(x):
		out = x
...
def GRDBlock(filters, growth_rate, rdb_count, dense_count):
	def func(x):
		out = x
		output_list = []
		...
		concate = Concatenate(axis = -1)(output_list)
		conv = Conv2D(filters = filters, \
					  kernel_size=(1,1), \
					  strides=(1, 1), \
					  padding='same', \
...
def GGRDBlock(filters, growth_rate, grdb_count, rdb_count, dense_count):
	def func(x):
		output = x
...
def get_grdn(patch_size):
	...
	model = Model(inputs=Inputs, outputs=Add1)
